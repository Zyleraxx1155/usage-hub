...
import { loadSettings, atomicWrite } from "./settings.js?v=0.8.0";
...
function kindOfSource(source) { return source?.kind === "quota" || source?.id === "codex" ? "quota" : "balance"; }
...
const SNAPSHOT_KEYS = ["sources", "updatedAt", "stale", "lastAttemptAt", "lastSuccessAt", "disabled", "cached"];
function safeSnapshot(value) {
  const out = {};
  for (const key of SNAPSHOT_KEYS) {
    if (!Object.prototype.hasOwnProperty.call(value || {}, key)) continue;
    out[key] = key === "sources" ? (Array.isArray(value.sources) ? value.sources.map(safeSource).filter(Boolean) : []) : value[key];
  }
  if (!Object.prototype.hasOwnProperty.call(out, "sources")) out.sources = [];
  return out;
}
...
export class BalanceService {
  constructor({ dataDir, fetchFn, networkFetch, homeDir = os.homedir(), dataRoot = "", pathCandidates = [], pluginDir = "", providerCatalog = null, auth = null, config = null, log } = {}) {
    ...
    this._kindInflight = { balance: null, quota: null }; // 余额/额度各自独立 in-flight
    this._committing = Promise.resolve(); // 串行化「合并 + 落盘」
  ...
  loadLastGood() {
    try {
      ...
      const sources = (value.sources || [])
        ...
        .filter(Boolean);
      return safeSnapshot({ ...value, sources });
    } catch { return null; }
  }
  snapshot() { return this.lastGood ? safeSnapshot({ ...this.lastGood, cached: this.lastGood.disabled ? false : true }) : { sources: [], updatedAt: null, cached: false, stale: true, lastAttemptAt: null, lastSuccessAt: null }; }
  // 三类刷新入口：refresh() 兼容旧的“整包刷新”（余额 + 额度），refreshBalance()/refreshQuota() 只刷新单一来源。
  // 单类刷新只请求对应上游，并在合并写回时保留另一类的已有快照，不覆盖/清空另一类状态。
  async refresh() {
    if (this.inflight) return this.inflight;
    this.inflight = Promise.all([this._refreshKind("balance"), this._refreshKind("quota")])
      .then(() => this.lastGood)
      .finally(() => { this.inflight = null; });
    return this.inflight;
  }
  refreshBalance() { return this._refreshKind("balance"); }
  refreshQuota() { return this._refreshKind("quota"); }

  // 每类一条独立 in-flight：余额与额度并发刷新互不阻塞、互不复用。
  _refreshKind(kind) {
    if (this._kindInflight[kind]) return this._kindInflight[kind];
    this._kindInflight[kind] = this._runKind(kind).finally(() => { this._kindInflight[kind] = null; });
    return this._kindInflight[kind];
  }

  async _runKind(kind) {
    const context = await this._loadContext();
    if (!context.settings.balance.enabled) return this._commitDisabled();
    const fetched = kind === "quota" ? await this._collectQuota(context) : await this._collectBalance(context);
    return this._commitKind(kind, fetched);
  }
  ...
  async _loadContext() {
    ...
    return { settings, credentials, roots, catalog, authCatalog, codexEnabled };
  }

  _previousMap() {
    return new Map((this.lastGood?.sources || []).map((source) => [source.id, safeSource(source)]).filter(([, source]) => source));
  }

  _makeRun(list, previous) {
    return async (id, configured, fn, kind = "balance") => {
      const label = kind === "quota" ? (id === "codex" ? "ChatGPT" : labelOf(id)) : labelOf(id);
      if (!configured) { list.push({ id, kind, label, status: "unavailable", configured: false }); return; }
      try { list.push({ id, kind, label, status: "ok", configured: true, ...await fn() }); }
      catch (error) {
        const status = statusOf(error);
        const old = previous.get(id);
        this.log?.warn?.(`[usage-hub] balance ${id} ${status}`);
        list.push({ ...(old || {}), id, kind, label, status, configured: true, stale: true });
      }
    };
  }

  // 供应商余额：配置驱动（balance-apis.json 可覆盖），key 一律取宿主 provider-catalog.json
  async _collectBalance({ credentials, roots, catalog }) {
    const sources = [];
    const run = this._makeRun(sources, this._previousMap());
  ...
  // Codex/GPT 订阅额度：独立上游，未启用时只产生 disabled 占位，不发任何请求。
  async _collectQuota({ authCatalog, codexEnabled }) {
    const sources = [];
    const run = this._makeRun(sources, this._previousMap());
    if (!codexEnabled) { sources.push({ id: "codex", kind: "quota", label: "ChatGPT", status: "unavailable", configured: false, disabled: true }); return sources; }
  ...
  // 合并写回：只替换本次刷新的那一类，另一类沿用 last-good，避免单类刷新覆盖另一类。
  _commitKind(kind, fetched) {
    return this._enqueueCommit(() => {
      const previous = this.lastGood;
      const preserved = (previous?.sources || []).filter((source) => kindOfSource(source) !== kind);
      const sources = kind === "quota" ? [...preserved, ...fetched] : [...fetched, ...preserved];
      const attemptAt = new Date().toISOString();
      const success = sources.some((source) => source.status === "ok");
      const result = { sources, updatedAt: attemptAt, cached: false, stale: sources.some((source) => source.status !== "ok"), lastAttemptAt: attemptAt, lastSuccessAt: success ? attemptAt : (previous?.lastSuccessAt || null) };
  ...
  _commitDisabled() {
    return this._enqueueCommit(() => {
      const attemptAt = new Date().toISOString();
      const result = { sources: [], updatedAt: attemptAt, cached: false, disabled: true, stale: false, lastAttemptAt: attemptAt, lastSuccessAt: this.lastGood?.lastSuccessAt || null };
      this.lastGood = result;
      atomicWrite(path.join(this.dataDir, "balance.json"), result);
      return result;
    });
  }
  // 串行化「合并 + 落盘」：余额/额度并发刷新时按队列提交，避免互相覆盖。
  _enqueueCommit(runCommit) {
    const next = this._committing.then(runCommit, runCommit);
    this._committing = next.then(() => {}, () => {});
    return next;
  }
}
