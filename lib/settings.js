...
export const DEFAULT_SETTINGS = Object.freeze({
  ...
  balance: { enabled: true, pollSeconds: 900, codexEnabled: true },
});
export const LIMITS = Object.freeze({ uiRefreshSeconds: [10, 3600], pollSeconds: [60, 86400] });
...
const allowed = { display: ["hideAgent", "hideModel", "hiddenAgents", "hiddenModels"], ui: ["refreshSeconds"], balance: ["enabled", "pollSeconds", "codexEnabled"], credentials: ["deepseekApiKey", "moonshotApiKey"] };
export function validateAndSave(dataDir, payload) {
  ...
  for (const section of ["display", "ui", "balance"]) {
    ...
    for (const key of Object.keys(payload[section])) {
      ...
      const numeric = (section === "ui" && key === "refreshSeconds") || (section === "balance" && key === "pollSeconds");
      if (list) {
