...
function parseContent(content, limitTurns = DEFAULT_TURN_LIMIT, { full = false } = {}) {
  ...
  for (const event of events) {
    ...
    turns.push({ index: turns.length + 1, timestamp: event.timestamp || null, model: event.message.model || event.modelId || model || null, provider: event.message.provider || event.provider || provider || null, inputTokens: usage.input, outputTokens: usage.output, cacheReadTokens: usage.cacheRead, cacheWriteTokens: usage.cacheWrite, reasoningTokens: usage.reasoning, totalTokens: usage.total ?? usage.input + usage.output + usage.cacheRead, hitRatio, hitRatioSource: usage.nativeHitRatio == null ? "jsonl-local" : "jsonl-native", contextWindow: usage.contextWindow, contextTokens: usage.contextTokens });
  }
  ...
  return { sessionId: session.id || null, agent: session.agentId || session.agent || null, title: firstUserTitle(events), startTime, endTime, model: model || last?.model || null, provider: provider || last?.provider || null, turns: full ? turns : turns.slice(-boundedLimit(limitTurns, DEFAULT_TURN_LIMIT, MAX_TURN_LIMIT)), turnCount: turns.length, inputTokens, outputTokens, cacheReadTokens, totalTokens, hitRatio: native ? (turns.at(-1)?.hitRatio ?? null) : (denominator ? cacheReadTokens / denominator : null), hitRatioSource: native ? "jsonl-native" : (derived ? "jsonl-local" : null), contextWindow, contextTokens, contextPercent: contextTokens == null || contextWindow == null || contextWindow <= 0 ? null : (contextTokens / contextWindow) * 100, parseWarnings: invalidLines.length ? { malformedLines: invalidLines.filter((x) => x === "malformed").length, eventLimit: invalidLines.includes("event-limit") } : null };
}

function safeRead(fullPath, limitTurns, warn, { full = false } = {}) {
  try {
    ...
    const parsed = parseContent(fs.readFileSync(fullPath, "utf8"), limitTurns, { full });
...
export function readSessionDetail({ sessionsDir, sessionsDirs, agent, sessionId, file, limitTurns = DEFAULT_TURN_LIMIT, full = false, warn } = {}) {
  ...
  const item = safeRead(path.join(dir, name), boundedLimit(limitTurns, DEFAULT_TURN_LIMIT, MAX_TURN_LIMIT), warn, { full });
  if (!item) return null;
