...
import { inputOf, uncachedOf, outputOf, reasoningOf, cacheReadOf, totalTokensOf, entryDay, entryHour, agentOf } from "./aggregate.js?v=0.6.0";
import { sourceTypeOf } from "./types.js?v=0.6.0";
import { ledgerSpeedRecords } from "./speed-scan.js?v=0.6.0";
...
export function makeRollupBucket() {
  return { totalTokens: 0, input: 0, uncached: 0, output: 0, reasoning: 0, cacheRead: 0, calls: 0, errors: 0 };
}
...
export function addEntryToRollup(rollup, e) {
  ...
  if (hour != null) { const hb = ensureHour(d.hours, hour); addTo(hb, e); addTo(ensure(hb.byType, type), e); }
  addTo(ensure(d.byType, type), e);
  // agent 维度带交叉表（byType/byModel/byProvider），支持「按 agent 筛选」下的分布卡
  const ab = ensureAgentBucket(d.byAgent, agent);
  addTo(ab, e); addTo(ensure(ab.byType, type), e); addTo(ensure(ab.byModel, model), e); addTo(ensure(ab.byProvider, provider), e);
  addTo(ensure(d.byModel, model), e);
  addTo(ensure(d.byProvider, provider), e);
  ...
  if (file) {
    ...
    addTo(rs.days[day].total, e);
    if (hour != null) addTo(ensure(rs.days[day].hours, hour), e);
  }
  return true;
}
...
/** 从明细全量构建 rollup（迁移用）。speeds 为可选的额外速度记录（如 JSONL 扫描结果）。 */
